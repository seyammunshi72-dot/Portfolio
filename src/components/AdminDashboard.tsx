                ...
                <div className="w-full bg-[#050505] border border-white/10 rounded-2xl overflow-hidden relative shadow-[0_0_50px_rgba(0,0,0,0.5)] mx-auto aspect-video cursor-grab active:cursor-grabbing">
                  <Canvas camera={{ position: [0, 1, 10], fov: 45 }} dpr={[1, 2]}>
                    ...
                    <OrbitControls 
                      ...
                      target={[0, 0, 0]}
                    />
                    <ErrorBoundary fallbackRender={({error}) => <Html center position={[0, 0, 0]}><div style={{color: 'white', background: 'rgba(255, 0, 0, 0.8)', padding: '10px', borderRadius: '4px', whiteSpace: 'nowrap'}}>3D model failed to load: {error?.message}</div></Html>}>
